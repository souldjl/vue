...
<script type="text/javascript" src="./js/live_dev.js?v=2"></script>
<script>
$(function(){
        ...
        success: function(data){
            ...
            if (data.on) {
                $.each(data.on,function(key,value){
                    ...
                    $(".livelist ul").append("<li class='on_zb' live_type='vod' endtime='"+value.end_time+"'><span><em>"+value.start_time+"</em>-"+value.end_time+"</span><a href='?vodUrl="+value.replay_id+"&secret="+value.secret+"'>观看录播</a><p>"+value.title+"</p></li>");
                    else if (url == value.room_id && !value.replay_id)
                    $(".livelist ul").append("<li class='on_zb' live_type='live' endtime='"+value.end_time+"'><span><em>"+value.start_time+"</em>-"+value.end_time+"</span><a href='?liveUrl="+value.room_id+"&secret="+value.secret+"'>正在直播</a><p>"+value.title+"</p></li>");
                    else if(value.room_id && value.replay_id)
                    $(".livelist ul").append("<li class='zbing' live_type='vod' endtime='"+value.end_time+"'><span><em>"+value.start_time+"</em>-"+value.end_time+"</span><a href='?vodUrl="+value.replay_id+"&secret="+value.secret+"'>观看录播</a><p>"+value.title+"</p></li>");
                    else
                    $(".livelist ul").append("<li class='zbing' live_type='live' endtime='"+value.end_time+"'><span><em>"+value.start_time+"</em>-"+value.end_time+"</span><a href='?liveUrl="+value.room_id+"&secret="+value.secret+"'>正在直播</a><p>"+value.title+"</p></li>");
                });
            }
            if (data.no) {
                $.each(data.no,function(key,value){
                    $(".livelist ul").append("<li class='no_zb'><span><em>"+value.start_time+"</em>-"+value.end_time+"</span><a href='javascript:void(0)'>马上开始</a><p>"+value.title+"</p></li>");
                });
            }

            if($('.livelist ul li').hasClass('on_zb')){
                if ($('.livelist ul .on_zb').attr('live_type') == 'vod') {
