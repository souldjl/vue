...
/*		if($(".class_fl li:nth-child(1) .current").attr("value")==12){
	...
	//鼠标经过fxrk时出现下拉
	$('.fxrk').hover(function() {	
		$('.fxList').css('display', 'block');
	}, function() {
		$('.fxList').css('display', 'none');
	});
	
	$('#sqbtn').click(function(event) {
		var txt=$(this).html();
		if (txt=="收起") {
		$('.class_fl').css({height:0,padding:0})
		$(this).html("展开").css('backgroundPosition', '-162px 0');
		} else{
			$('.class_fl').css({height:'auto',padding:'15px 0 15px 25px'})
			$(this).html("收起").css('backgroundPosition', '0 0');
		};
	});	
})